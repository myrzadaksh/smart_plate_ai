...
export interface DummyPreferences {
    user_id: number;
...
export interface Stats {
    recipes: {
        ...
        cuisine_types_count: number;
        avg_cook_time: number;
    };
    pantry: {
        total_items: number;
        total_categories: number;
        running_low_count: number;
...
export const dummyPreferences: DummyPreferences = {
    user_id: 1,
...
export const dummyShoppingListItems: ShoppingListItem[] = [
    {
        id: 1,
        user_id: 1,
...
export const dummyGeneratedRecipe: GeneratedRecipe = {
...
export const getExpiringItems = (): PantryItem[] => {
...
export const getRecentRecipes = (limit: number = 5): Recipe[] => {
    return [...dummyRecipes]
...
export const getUpcomingMeals = (limit: number = 5): MealPlan[] => {
    const today = new Date().toISOString().split('T')[0];
    return dummyMealPlans
        .filter(meal => meal.meal_date >= today)
        .sort((a, b) => new Date(a.meal_date).getTime() - new Date(b.meal_date).getTime())
        .slice(0, limit);
};

// Get recipe by ID
export const getRecipeById = (id: string | number): Recipe | undefined => {
    const numericId = typeof id === 'string' ? parseInt(id, 10) : id;
    return dummyRecipes.find(recipe => recipe.id === numericId);
};

// Simulate async data fetching
export const simulateApiCall = <T>(data: T, delay: number = 500): Promise<{ data: { data: T } }> => {
